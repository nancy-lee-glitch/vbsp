    ...
    try {
      const response = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          ssnLast4: onboardSsn ? onboardSsn.slice(-4) : '4412'
        }),
      });
      ...
      if (data.success && data.user) {
        createdUserAccount = {
          ...
          name: data.user.name || data.user.full_name || onboardName.trim(),
          email: data.user.email || onboardEmail.trim(),
          accountNumber: data.user.accountNumber || data.user.account_number || generatedAccountNum,
          thriftlinePin: data.user.thriftlinePin || data.user.thriftline_pin || generatedPin,
          phone: data.user.phone || onboardPhone || '(202) 555-0149',
          address: data.user.address || '400 7th St SW, Washington, DC 20024',
          employingAgency: data.user.employingAgency || data.user.employing_agency || onboardAgency || 'Department of Defense (DoD)',
          planType: data.user.planType || data.user.account_type || onboardPlanType,
          hireDate: data.user.hireDate || new Date().toISOString().split('T')[0],
          totalBalance: Number(data.user.totalBalance ?? data.user.total_balance ?? 0),
          traditionalBalance: Number(data.user.traditionalBalance ?? data.user.traditional_balance ?? 0),
          rothBalance: Number(data.user.rothBalance ?? data.user.roth_balance ?? 0),
          ytdReturn: Number(data.user.ytdReturn ?? 0),
